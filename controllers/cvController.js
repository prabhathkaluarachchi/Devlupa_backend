...
exports.generateReport = async (req, res) => {
  try {
    ...
    const addFooter = () => {
      // Add footer at the bottom of the current content, not fixed position
      const footerY = Math.max(yPosition + 20, doc.page.height - 30);
      doc
        .fillColor("#9CA3AF")
        .fontSize(8)
        .font("Helvetica")
    ...
    // Function to check if we need new page and add footer to current page
    const checkNewPage = (requiredHeight = 0) => {
      if (yPosition + requiredHeight > 650) {
        // Reduced from 700 to account for footer
    ...
    doc.fillColor("#374151").fontSize(9).font("Helvetica"); // Smaller font

    summaryItems.forEach((item, index) => {
      doc.text(`• ${item}`, 55, yPosition + index * 12); // Reduced line height
    });

    yPosition += summaryItems.length * 12 + 15; // Reduced spacing

    // Check if we have enough space for first section, otherwise add footer and new page
    checkNewPage(40); // Check if we need new page for first section
    ...
    let hasContentOnPage = yPosition > 50; // Track if we have content on current page
    ...
    sections.forEach((section, sectionIndex) => {
      ...
      if (hasContentOnPage && sectionIndex > 0) {
        yPosition += 3; // Minimal space between sections
      }
      ...
      doc
        .roundedRect(50, yPosition, 495, 18, 2) // Reduced height
      ...
      // Section content with INCREASED GAP BETWEEN CANDIDATES
      section.data.forEach((candidate, index) => {
        // Calculate exact height needed - INCREASED LINE HEIGHT
        let candidateHeight = 28; // INCREASED from 22px for better readability
        ...
        if (
          ...
          candidateHeight = 34; // INCREASED from 26px for better readability
        } else if (section.title === "ANALYSIS ERRORS") {
          candidateHeight = 38; // INCREASED from 30px for better readability
        }

        // Check if we need new page
        checkNewPage(candidateHeight);

        // Candidate item with BETTER LINE HEIGHT
        doc
          .fillColor("#111827")
          .fontSize(8)
          .font("Helvetica-Bold")
        ...
        // Score - with BETTER LINE HEIGHT
        doc
          .fillColor("#6B7280")
          .fontSize(7)
          .font("Helvetica")
          .text(`Score: ${candidate.matchScore}%`, 55, yPosition + 12); // INCREASED from 9px

        // Email for invitations sent - with BETTER LINE HEIGHT
        if (
          section.title === "INVITATIONS SENT" &&
          emailInputs &&
          emailInputs[candidate.fileName]
          ...
          doc
            ...
            .text(
              ...
              55,
              yPosition + 24 // INCREASED from 18px
            );
        }
        // Error message for analysis errors - with BETTER LINE HEIGHT
        else if (section.title === "ANALYSIS ERRORS") {
          doc
            .fillColor("#EF4444")
            .fontSize(7)
        ...
        if (index < section.data.length - 1) {
          yPosition += 8; // ADDED 8px gap between candidate items
